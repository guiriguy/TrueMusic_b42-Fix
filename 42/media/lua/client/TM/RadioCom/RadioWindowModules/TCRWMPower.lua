...
function TCRWMPower:createChildren()
    ...
    self.itemDropBox = ISItemDropBox:new(xoff + 10, 4, self.height - 8, self.height - 8, false, self,
        TCRWMPower.addBattery, TCRWMPower.removeBattery, TCRWMPower.verifyItem, nil);
...
function TCRWMPower:update()
    ...
    local hasBattery = self.deviceData:getHasBattery()
    local power = self.deviceData:getPower() or 0

    -- Visual State (button/label)
    self.led:setLedIsOn(isOn)
    self.toggleOnOffButton:setTitle(getText(isOn and "IGUI_DeviceOff" or "IGUI_DeviceOn"))

    -- Battery info
    self.batteryStatus:setPower(power)
