...
function TCRWMMedia:createChildren()
    ...
    self.itemDropBox = ISItemDropBox:new(x, y, 24, 24, false, self, TCRWMMedia.addMedia, TCRWMMedia.removeMedia,
        TCRWMMedia.verifyItem, nil);
    ...
    self.toggleOnOffButton = ISButton:new(10, y, self:getWidth() - 20, btnHgt, getText("ContextMenu_Turn_On"), self,
        TCRWMMedia.togglePlayMedia);
...
function TCRWMMedia:connectSpeaker(_item, dx, dy)
    local square = TM.Devices.Square.get(self.device, self.deviceType)
...
function TCRWMMedia:togglePlayMedia()
    if self:doWalkTo() then
        ...
        if not self.device:getModData().tcmusic.needSpeaker or self.device:getModData().tcmusic.connectTo then
            ...
            if TCRWMMedia.connectSpeaker(self.device, 1, 1) then
...
function TCRWMMedia:verifyItem(_item)
    local tmMusic = self.device:getModData() and self.device:getModData().tcmusic
    local existing = tmMusic and tmMusic.mediaItem
    local playerTag = tmMusic and tmMusic.playerTag
    return TM.Media.canInsert(self.deviceData, _item, existing, playerTag)
end
...
function TCRWMMedia:update()
    ...
    if (not isOn) and tmMusic.mediaItem and self.device:getDeviceData():getEmitter() and self.device:getDeviceData():getEmitter():isPlaying(tmMusic.mediaItem) then
    ...
    if tmMusic.deviceType == "VehiclePart" then
        ...
        active = TM.State.Active and TM.State.Active[key]
    ...
    elseif tmMusic.deviceType == "InventoryItem" then
        ...
        if active then
            ...
            if tmMusic.needSpeaker and not tmMusic.connectTo then
    ...
    if active or (tmMusic.deviceType == "VehiclePart" and self.device:getVehicle():getEmitter() and self.device:getVehicle():getEmitter():isPlaying(tmMusic.mediaItem)) then
    ...
    elseif button == Joypad.BButton then
        if self.device:getModData().tcmusic.mediaItem then
            ...
            if self.device:getModData().tcmusic.deviceType == "InventoryItem" then
                musicPlayer = TCMusic.ItemMusicPlayer[self.device:getFullType()] or
                    TCMusic.WalkmanPlayer[self.device:getFullType()]
